...
public class getPrecision {
    ...
    public HashSet<String> getAnchors() throws IOException {
        ...
        String anchors_file = "D:\\论文\\database\\anchor\\groundtruth.8.foldtrain.test.number";
    ...
    public HashSet<String> getTrainAnchors() throws IOException {
        HashSet<String> anchors_set = new HashSet<String>();
        String anchors_file ="D:\\论文\\database\\anchor\\groundtruth.8.foldtrain.train.number";
    ...
    public TreeMap<Integer, Integer> getTopSimilarity(int k,int index) throws IOException {
        TreeMap<Integer, Integer> answer = new TreeMap<Integer, Integer>();
        for (int i = 0; i < k; i++) {
            answer.put(i + 1, 0);
        }
        HashSet<String> anchors = getAnchors();
        String myspace = "D:\\论文\\database\\twitter\\emb_train\\8\\tw.IONEembedding."+index+".num";
                //"D:\\论文\\database\\twitter\\emb_train\\8\\tw.INEembedding."+index+".100_dim.10000000";
        String lastfm ="D:\\论文\\database\\foursquare\\emb_train\\8\\four.IONEembedding."+index+".num";
                //"D:\\论文\\database\\foursquare\\emb_train\\8\\four.INEembedding."+index+".100_dim.10000000";
    ...
    public TreeMap<Integer, Integer> getTopSimilarityReverse(int k,int index) throws IOException {
        TreeMap<Integer, Integer> answer = new TreeMap<Integer, Integer>();
        for (int i = 0; i < k; i++) {
            answer.put(i + 1, 0);
        ...
        HashSet<String> anchors = getAnchors();
        String lastfm =  "D:\\论文\\database\\twitter\\emb_train\\8\\tw.IONEembedding."+index+".num";
                //"D:\\论文\\database\\twitter\\emb_train\\8\\tw.INEembedding."+index+".100_dim.10000000";
        String myspace ="D:\\论文\\database\\foursquare\\emb_train\\8\\four.IONEembedding."+index+".num";
                // "D:\\论文\\database\\foursquare\\emb_train\\8\\four.INEembedding."+index+".100_dim.10000000";
        HashMap<String, double[]> myspace_embedding = getUserEmbeddingTest(myspace);
        HashMap<String, double[]> lastfm_embedding = getUserEmbeddingTest(lastfm);
        for (String uid : myspace_embedding.keySet()) {
    ...
    public void getFinalAnswer(int index) throws IOException {
        /*���Լ��ĵ�*/
        HashSet<String> anchors = getAnchors();
        double total = anchors.size() * 2;
        for (int i = 10000000; i <= 10000000; i += 10000000) {
            //String file_postfix=embedding_type+"."+i;
            TreeMap<Integer, Integer> answer_1 = getTopSimilarity(30,index); //twitter find foursquare
            //System.out.println("===========================");
            TreeMap<Integer, Integer> answer_2 = getTopSimilarityReverse(30,index);//foursquare find twitter
